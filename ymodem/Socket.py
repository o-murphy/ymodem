...
class ModemSocket(Channel):
    ...
    def set_protocol(
        ...
        if self.protocol_type == ProtocolType.YMODEM:
            if (
                "g" in protocol_type_options
                and (self._protocol_features & YMODEM.ALLOW_YMODEM_G) != 0
            ):
    ...
    def send(
        ...
        # XYMODEM process
        if (
            self.protocol_type == ProtocolType.XMODEM
            or self.protocol_type == ProtocolType.YMODEM
        ):
            ...
            for task_index, task in enumerate(tasks):
                try:
                    ...
                    self.logger.error(
                        f"[Sender]: Cannot open the file: {task.path}, skip."
                    )
                    continue
                ...
                if self.protocol_type == ProtocolType.YMODEM:
                    ...
                    if c:
                        ...
                        self.logger.error(
                            "[Sender]: Waiting for command from Receiver has timed out, abort and exit!"
                        )
                        self._abort()
                        self.logger.debug("[Sender]: CAN ->")
                        if stream:
                    ...
                    self.logger.debug(
                        f"[Sender]: {'SOH' if self._packet_size == 128 else 'STX'} ->"
                    )
                ...
                while True:
                    try:
                        ...
                        self.logger.error(
                            "[Sender]: Failed to read file, abort and exit!"
                        )
                        self._abort()
                        self.logger.debug("[Sender]: CAN ->")
                        if stream:
                    ...
                    while True:
                        if (
                            self.protocol_type == ProtocolType.XMODEM
                            or self.protocol_subtype
                            == ProtocolSubType.YMODEM_BATCH_FILE_TRANSMISSION
                        ):
                            if retries < 10:
                                self.write(header + data + checksum)
                                self.logger.debug(
                                    f"[Sender]: Data packet {sequence} ->"
                                )
                                ...
                                if c:
                                    ...
                                    if callable(callback):
                                        callback(
                                            task_index, task.name, task.total, task.sent
                                        )
                                    break
                                else:
                                    self.logger.warning(
    ...
    def recv(
        self, path: str, callback: Optional[Callable[[int, str, int, int], None]] = None
    ) -> bool:
        # XYMODEM process
        if (
            self.protocol_type == ProtocolType.XMODEM
            ...
            while True:
                ...
                if self.protocol_type == ProtocolType.YMODEM:
                    ...
                    while True:
                            ...
                            if data and len(data) == (packet_size + 2):
                                ...
                                if valid:
                                    file_name = bytes.decode(
                                        data.split(b"\x00")[0], "utf-8"
                                    )

                                    # batch end packet received
                                    if not file_name:
                                        self.logger.debug(
                                            "[Receiver]: <- Batch end packet"
                                        )
                                        if (
                                            self.protocol_subtype
                                            == ProtocolSubType.YMODEM_BATCH_FILE_TRANSMISSION
                                    ...
                                    if (
                                        ...
                                        task.total = int(
                                            data
                                            if space_index == -1
                                            else data[:space_index]
                                        )
                                        self.logger.debug(
                                            f"[Receiver]: Size - {task.total} bytes"
                                        )
                                    ...
                                    if self._protocol_features & YMODEM.USE_DATE_FIELD:
                                        ...
                                        self.logger.debug(
                                            f"[Receiver]: Mtime - {task.mtime} seconds"
                                        )
                                        data = data[space_index + 1 :]
                        ...
                        if (
                            ...
                            self.protocol_subtype
                            == ProtocolSubType.YMODEM_G_FILE_TRANSMISSION
                            and not received
                        ):
                            """
                            ...
                            try:
                                ...
                                self.logger.error(
                                    f"[Receiver]: Cannot open the save path: {p}, abort and exit!"
                                )
                                self._abort()
                                self.logger.debug("[Receiver]: CAN ->")
                                if stream:
                ...
                if (
                    ...
                    self.logger.warning(
                        "[Receiver]: No response in crc mode, try checksum mode..."
                    )
                    for _ in range(10):
                ...
                if not c:
                    self.logger.error(
                        "[Receiver]: No response in checksum mode, abort and exit!"
                    )
                    self._abort()
                    self.logger.debug("[Receiver]: CAN ->")
                    if stream:
                ...
                while True:
                    ...
                    if seq1 == seq2 == sequence:
                        ...
                        if data and len(data) == (packet_size + 1 + crc):
                            ...
                            if valid:
                                self.logger.debug(
                                    f"[Receiver]: <- Data packet {sequence}"
                                )
                                ...
                                if callable(callback):
                                    callback(
                                        task_index, task.name, task.total, task.received
                                    )
                    ...
                    # invalid header: expired sequence
                        ...
                        self.logger.warning(
                            "[Receiver]: Expired sequence, drop the whole packet."
                        )
    ...
    def _verify_recv_checksum(self, crc, data):
        if crc:
            ...
            if not valid:
                self.logger.debug(
                    "[Receiver]: CRC verification failed. Sender: %04x, Receiver: %04x.",
                    remote_sum,
                    local_sum,
                )
        else:
            ...
            if not valid:
                self.logger.debug(
                    "[Receiver]: CRC verification failed. Sender: %02x, Receiver: %02x.",
                    remote_sum,
                    local_sum,
                )
        return valid, data
